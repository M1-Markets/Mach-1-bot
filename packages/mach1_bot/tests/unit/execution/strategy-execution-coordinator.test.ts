...
describe("StrategyExecutionCoordinator", () => {
  ...
  it("supports manual logical-clock scheduling", async () => {
    ...
    await expect(coordinator.executeIfIntervalElapsed(1_000)).resolves.toBe(
      true,
    );
    await expect(coordinator.executeIfIntervalElapsed(5_999)).resolves.toBe(
      false,
    );
    await expect(coordinator.executeIfIntervalElapsed(6_000)).resolves.toBe(
      true,
    );
  ...
  it("retries manual scheduling when executor skips tick", async () => {
    ...
    await expect(coordinator.executeIfIntervalElapsed(1_001)).resolves.toBe(
      true,
    );
