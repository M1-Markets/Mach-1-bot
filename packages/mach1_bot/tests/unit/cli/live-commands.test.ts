...
vi.mock("@/cli/utils/live-utils", () => ({
  ...
  formatStepAlignedAmount: vi.fn(() => "1"),
  ...
  withMonacoSession: vi.fn(
    async (
      ...
      const sdk = {
        ...
        marginAccounts: {
          getParentMarginAccountSummary: async () => ({
          ...
          transferCollateralFromMarginAccount: async (
            ...
            status: "SUCCESS",
            newEquity: "1400",
            newTotalCollateralValue: "1400",
            newWithdrawableCollateral: "1000",
          }),
        },
        delegatedAgents: {
          ...
          upsertDelegatedAgent: async (request: {
            ...
            allowedTradingPairIds: request.allowedTradingPairIds,
            allowedMarginAccountIds: request.allowedMarginAccountIds ?? [],
            allowedOrderTypes: request.allowedOrderTypes,
            allowedTimeInForce: request.allowedTimeInForce,
...
describe("live subcommands", () => {
  ...
  it("runs live margin-account create", async () => {
    const program = new Command("live");
    registerLiveCommands(program);
    const code = await runCommand(program, ["margin-account", "create"]);
    expect(code).toBe(0);
    expect(logSpy).toHaveBeenCalledWith("Parent Margin Account");
    expect(logSpy).toHaveBeenCalledWith(
      JSON.stringify({
        marginAccountId: "margin-2",
        equity: "0",
